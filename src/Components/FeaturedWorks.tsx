    ...
    <section className="bg-black text-white py-20 px-6 md:px-20 xl:mx-[140px]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
        {projects.map((p, i) => (
          <div
            key={i}
            className="relative rounded-2xl overflow-hidden shadow-lg group cursor-pointer border border-2px border-white"
          >
            {/* IMAGE */}
            <img
              src={p.image}
            ...
            <div className="absolute bottom-6 left-6 right-6">
              <h2 className="text-[#F6BE00] font-bold text-sm uppercase">
                {p.title}
              </h2>
